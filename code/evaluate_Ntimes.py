    ...
    for fold in range(1, args.folds+1):
        ...
        for idx, data_id in enumerate(tqdm(ids_list)):
            ...
            if args.task == "amos":
                ...
                image = read_nifti(os.path.join(config.base_dir, 'imagesTr', f'{data_id}.nii.gz'))
                label =read_nifti(os.path.join(config.base_dir, f'labelsTr', f'{data_id[:-3]}seg.nii.gz'))  
            ...
            image = F.interpolate(torch.Tensor(image), size=resize_shape, mode='nearest')
            image = image.squeeze().numpy()

            # Create a NIfTI image
            image = nib.Nifti1Image(image, np.eye(4))
            
            os.makedirs(os.path.join('./logs',f'image_{args.task}',args.exp), exist_ok=True)
            os.makedirs(os.path.join('./logs',f'label_{args.task}',args.exp), exist_ok=True)
            os.makedirs(os.path.join('./logs',f'pred_{args.task}',args.exp), exist_ok=True)
            
            #nib.save(image, os.path.join('./logs',f'image_{args.task}',args.exp,  f'{data_id}.nii.gz'))
            

            pred_s = torch.FloatTensor(pred).unsqueeze(0).unsqueeze(0)
            pred_s = F.interpolate(torch.Tensor(pred_s), size=resize_shape, mode='nearest')
            pred_s = pred_s.squeeze().numpy()
            pred_s = nib.Nifti1Image(pred_s, np.eye(4))
            nib.save(pred_s, os.path.join('./logs',f'pred_{args.task}',args.exp,  f'{data_id}.nii.gz'))
